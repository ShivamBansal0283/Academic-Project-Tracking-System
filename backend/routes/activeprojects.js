...
router.post('/', (req, res) => {
  ...
  const query = 'SELECT P.project_id,P.project_title FROM Project P JOIN Team T ON P.project_id=T.project_id JOIN Team_Members TM ON T.team_id=TM.team_id LEFT JOIN Task TS ON T.project_id=TS.project_id LEFT JOIN Submission S ON TS.task_id=S.task_id AND T.team_id=S.team_id WHERE TM.member_id=? GROUP BY P.project_id,P.project_title HAVING COUNT(TS.task_id)>COUNT(S.task_id) OR COUNT(S.task_id) IS NULL';
